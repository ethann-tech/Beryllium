package com.ethan.framework.widget
 ...
 * 16:9的视频封面图片 默认 16：9
 */
class VideoImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) : AppCompatImageView(context, attrs) {
    ...
    init {
        if (attrs != null) {
            ...
            ratio = array.getFloat(R.styleable.VideoImageView_ratio, 0.56f)
            widthOffset = array.getInt(R.styleable.VideoImageView_offset, 0)
            array.recycle()
        }
    }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        setBackgroundResource(R.color.transparent) // 父容器传过来的宽度方向上的模式
        val widthMode = MeasureSpec.getMode(widthSpec) // 父容器传过来的高度方向上的模式
        val heightMode = MeasureSpec.getMode(heightSpec) // 父容器传过来的宽度的值
        var width: Int = MeasureSpec.getSize(widthSpec) - paddingLeft - paddingRight - widthOffset.dp2px() // 父容器传过来的高度的值
