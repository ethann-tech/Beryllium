...
import com.ethan.glide.transformation.BlurTransformation
import com.ethan.glide.transformation.CircleBorderTransform
import io.github.uhsk.kit.android.screenWidth
import java.io.File
...
fun ImageView.setUrlErrorIcon(url: String?, @DrawableRes errorRes: Int) {
    if (ActivityManager.isActivityDestroy(context)) return
    Glide.with(context).load(url).placeholder(errorRes).error(errorRes).priority(Priority.HIGH).skipMemoryCache(true) //不启动缓存
...
fun ImageView.setBlurView(url: String?, radius: Int = 25, sampling: Int = 1) {
    if (ActivityManager.isActivityDestroy(context)) return //请求配置
...
fun ImageView.setScanImage(url: String?) {
    if (ActivityManager.isActivityDestroy(context)) return //获取bitmap根据Drawable设定图片显示尺寸
    Glide.with(context).asDrawable().load(url).placeholder(R.mipmap.default_img).skipMemoryCache(false).diskCacheStrategy(DiskCacheStrategy.DATA).error(R.mipmap.default_img).into(object : CustomTarget<Drawable?>() {
            override fun onResourceReady(resource: Drawable, transition: Transition<in Drawable?>?) {
                ...
                layoutParams = lp
                layoutParams = lp
                setImageDrawable(resource) //Glide.with(context).load(drawable).override(lp.width, lp.height).into(this)
            }
